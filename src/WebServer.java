...
final class HttpRequest implements Runnable {
	...
	private void processRequest() throws Exception {
		...
		if (method.equals("GET")) {
			...
			sendPacket = new DatagramPacket(temp.getBytes(), temp.getBytes().length, serverAddress, 5000);
			...
			while (count < (totalCount - headerLength - footerLength)
				...
				if (bytes < (totalCount - headerLength - footerLength) - count) {
					sendPacket = new DatagramPacket(data, bytes, serverAddress, 5000);
				} else {
					sendPacket = new DatagramPacket(data, (totalCount - headerLength - footerLength) - count, serverAddress, 5000);
				}
				count += bytes;
				socket.send(sendPacket);
			}
			...
			sendPacket = new DatagramPacket(endString.getBytes(), endString.getBytes().length, serverAddress, 5000);
			socket.send(sendPacket);
